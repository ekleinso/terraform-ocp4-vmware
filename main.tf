...
locals {
  ...
  bootstrap_fqdns     = [format(var.vm_name_format, "b", var.vm_name_start, var.base_domain)]
  control_plane_fqdns = [for idx in range(var.control_plane_count) : format(var.vm_name_format, "m", idx + var.vm_name_start, var.base_domain)]
  compute_fqdns       = [for idx in range(var.compute_count) : format(var.vm_name_format, "w", idx + var.vm_name_start, var.base_domain)]
  storage_fqdns       = [for idx in range(var.storage_count) : format(var.vm_name_format, "s", idx + var.vm_name_start, var.base_domain)]
  infra_fqdns         = [for idx in range(var.infra_count) : format(var.vm_name_format, "i", idx + var.vm_name_start, var.base_domain)]
...
module "ignition" {
  ...
  vsphere_folder      = var.vsphere_preexisting_folder ? local.folder_path : vsphere_folder.folder.0.path
  ...
  storage_nodes       = [for idx in range(length(local.storage_fqdns)) : element(split(".", element(local.storage_fqdns, idx)), 0)]
  infra_nodes         = [for idx in range(length(local.infra_fqdns)) : element(split(".", element(local.infra_fqdns, idx)), 0)]
}
...
module "bootstrap" {
  ...
  folder_id             = var.vsphere_preexisting_folder ? local.folder_path : vsphere_folder.folder.0.path
