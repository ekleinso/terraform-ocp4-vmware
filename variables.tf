...
// if vsphere_preexisting_resourcepool = true and vsphere_resource_pool = "" then the cluster resource pool is used
...
variable "vm_name_format" {
  description = "Starting string for vm_name"
  type        = string
  default     = "node-%s-%02d.%s"
}

variable "vm_name_start" {
  description = "Starting count for VMs"
  type        = number
  default     = 0
}
