...
data "template_file" "post_deployment_05" {
  template = templatefile("${path.module}/templates/99_05-post-deployment.yaml", {
    ...
    label_nodes        = base64encode(data.template_file.label_nodes.rendered)
  })
}
...
resource "null_resource" "generate_manifests" {
  provisioner "local-exec" {
    command = <<EOF
set -ex
cp ${local.installerdir}/install-config.yaml ${local.installerdir}/install-config.yaml.bak
...
data "template_file" "label_nodes" {
  template = <<EOF
#!/usr/bin/env bash

sleep 300

%{ for host in var.storage_nodes ~}
oc label node ${host} tf-role=storage
%{ endfor ~}

%{ for host in var.infra_nodes ~}
oc label node ${host} tf-role=infra
%{ endfor ~}
